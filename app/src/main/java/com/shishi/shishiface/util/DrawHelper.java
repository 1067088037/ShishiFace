package com.shishi.shishiface.util;
...
public class DrawHelper {
    ...
    public static void drawFaceRect(Canvas canvas, DrawInfo drawInfo, int color, int faceRectThickness) {
        ...
        if (drawInfo.getName().equals("伟大的领袖毛主席")) paint.setColor(Color.RED);
        ...
        if (drawInfo.getName().equals("")) {
            //绘制文字外描边
            paint.setStyle(Paint.Style.STROKE);
            paint.setTextSize(rect.width() / 8);
            paint.setColor(Config.DRAW_BACKGROUND_COLOR);
            String str = (drawInfo.getSex() == GenderInfo.MALE ? "男" : (drawInfo.getSex() == GenderInfo.FEMALE ? "女" : "未知性别")) + ",";
            if (drawInfo.getAge() != -1) {
                str = str + (drawInfo.getAge() == AgeInfo.UNKNOWN_AGE ? "未知年龄" : drawInfo.getAge()) + ",";
            }
            str = str + (drawInfo.getLiveness() == LivenessInfo.ALIVE ? "活体" : (drawInfo.getLiveness() == LivenessInfo.NOT_ALIVE ? "非活体" : "未知活体"));
            canvas.drawText(str, rect.left, rect.top - 25, paint);
            //绘制文字颜色
            Paint paint1 = new Paint();
            paint1.setStyle(Paint.Style.FILL);
            paint1.setTextSize(rect.width() / 8);
            if (color == Config.DRAW_COLOR) {
                paint1.setColor(Color.BLACK);
            } else {
                paint1.setColor(color);
            }
            canvas.drawText(str, rect.left, rect.top - 25, paint1);
        } else {
            paint.setStyle(Paint.Style.FILL_AND_STROKE);
            paint.setTextSize(rect.width() / 8);
            if (drawInfo.getName().equals("伟大的领袖毛主席")) paint.setColor(Color.RED);
            canvas.drawText(drawInfo.getName(), rect.left, rect.top - 25, paint);
        }
    }
